...
- **Frontend**: **Dashboard**, **projects** list, **surface** inventory, **hypotheses** drawer, **findings** inventory + drawer, **evidence** inventory + presign upload; dedicated policy/admin console still open.
- **Object storage**: MinIO wired in compose; API **presigns** **PUT** URLs (**`POST …/evidence/presign`**) and can **register** **`evidence_bundles`** metadata; server-side streaming upload of large bodies not implemented yet.
- **Hypotheses**: Stub generation; **approve** and **reject** (queued-only); web **drawer** with rationale and **supporting_evidence**; no ranking or RAG-backed proposals yet.
- **MCP / sub-agents**: **`GET /api/mcp/servers`** summarizes **`SENTINEL_MCP_CONFIG`** (names + transport); optional **`SENTINEL_MCP_DISABLED_SERVERS`** suppresses entries from that response. **Allowlist / risk-class model** documented in **`docs/architecture.md`**. No MCP client or audited tool-call path yet. **Config contract**: **`SENTINEL_MCP_CONFIG`** → JSON with **`mcpServers`** (Cursor-style); see **`config/mcp.example.json`**.
