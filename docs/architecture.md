...
- **API** (FastAPI): Serves REST endpoints; uses **Postgres** when `DATABASE_URL` is set (Docker Compose sets this), otherwise **in-memory** stores for local/tests. SQLAlchemy provides the **engine**; persistence uses **SQLAlchemy Core** (`text()`), not ORM models yet. **`POST /api/sync/requests`** persists **`caido_requests`** and upserts **`endpoints`** (method + path, **query string stripped** for grouping). **`POST /api/sync/findings`** stores **`findings`**; **`GET /api/projects/{id}/findings`** lists them. **`POST …/evidence/presign`** (when **`S3_*`** configured) returns a **presigned PUT** URL and server-chosen **`storage_key`**; **`POST/GET /api/projects/{id}/evidence`** registers **metadata** for objects in S3-compatible storage. Hypotheses **approve** / **reject** apply only while **`queued`**. **`GET /api/mcp/servers`** summarizes **`SENTINEL_MCP_CONFIG`** (no secrets). Optional **`SENTINEL_API_WRITES_DISABLED`** blocks mutating methods on **`/api/*`**. Optional **`SENTINEL_RATE_LIMIT_RPM`** applies an in-process sliding window on **`/api/*`** only. Emits structured JSON log lines for selected events.
...
- **Web**: Next.js App Router; **production image** uses standalone output. Browser calls API via `NEXT_PUBLIC_API_BASE_URL` (Compose defaults to host port **30880**). Minimal UI: **dashboard**, **`/admin`** (policy / ops flags), **projects** (create + list), **surface inventory**, **hypotheses** (drawer; approve / reject), **findings** (filters, export, drawer), **evidence** (presign upload; filters, export).
- **Compose**: Internal service DNS (`postgres`, `redis`, `minio`); **published host ports** are configurable via `infra/docker/.env` to avoid clashes with local software. API and worker **bind-mount** a host MCP JSON file (default **`config/mcp.example.json`**) to **`/etc/sentinel/mcp.json`** (`SENTINEL_MCP_HOST_FILE`).
...
**MCP server and tool allowlist (target model):**

1. **Deny-by-default for tools:** Config may define which servers exist, but **which tools** each sub-agent may call must be an explicit allowlist (names or patterns), not “all tools on server.”
2. **Global vs per-project:** A **global** allowlist (env, config file, or vault-backed) caps the universe of servers/tools; **per-project** rows (future DB) may further restrict tools for engagements with tighter scope. Evaluated as **intersection** (most restrictive wins).
3. **Secrets:** MCP **`headers`** and **`env`** in JSON must not be committed; load from a **vault** or inject at deploy time; **`GET /api/mcp/servers`** never returns them.
4. **Risk classes (design):** classify tools as **read**, **write**, or **destructive**; require human approval + audit for **write** and above; block **destructive** unless an explicit break-glass policy exists.
5. **Operational visibility:** **`SENTINEL_MCP_DISABLED_SERVERS`** (comma-separated **`mcpServers`** keys) removes servers from **`GET /api/mcp/servers`** and increments **`suppressed_server_count`**. Until the MCP **client** ships, this does **not** block tool execution—it is for dashboards and operator hygiene; the client must apply the same list (or stronger) before any call.
